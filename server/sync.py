...
from frameioclient import Utils
...
class SyncLoop(Thread):
    def __init__(self):
        ...
        self.Project, self.Asset, self.IgnoreFolder = init_sync_models(self.db)[1:]
    ...
    def update_projects(self):
        ...
        for project in projects:
            try:
                db_project = self.Project.get(self.Project.project_id == project["id"])
    ...
    def calculate_missing_paths(
        self, project, parent_id, parent_path, ignore_folders, ignore
    ):
        ...
        for child in children:
            if (
                child.name in ignore_folders
                or ignore
                or self.wildcard_match(child.name, ignore_folders)
            ):
                ignore = True
            ...
            self.calculate_missing_paths(
                project, child.asset_id, child.path, ignore_folders, ignore
            )
    ...
    def update_frameio_assets(self, project, ignore_folders):
        ...
        for folder in new_folders_filtered:
            ...
            if folder["parent_id"] == project.root_asset_id:
                ...
                try:
                    parent = self.Asset.get(self.Asset.asset_id == folder["parent_id"])
            ...
            try:
                self.Asset.get(
                    self.Asset.path == path, self.Asset.project_id == project.project_id
                )
                ignore = True
            ...
            if file["upload_completed_at"] is not None:
                ...
                if file["parent_id"] == project.root_asset_id:
                    ...
                    try:
                        ...
                        logger.info(
                            "Parent to {} not found, retry".format(file["name"])
                        )
                        continue
        ...
        if added_folders - duplicates_folders != 0:
            logger.info("New folders on Frame.io for project {}".format(project.name))
        if added_files - duplicates_files != 0:
            logger.info("New files on Frame.io for project {}".format(project.name))
    ...
    def update_local_assets(self, project, ignore_folders):
        ...
        if not os.path.isdir(abs_project_path):
            logger.info(
                "Local folder for {} not found, turning off sync".format(project.name)
            )
        ...
        if new_folders:
            logger.info("New local folders for project {}".format(project.name))
        if new_files:
            logger.info("New local files for project {}".format(project.name))
    ...
    def download_new_assets(self, project):
        ...
        for folder in new_folders:
            logger.info(
                "Creating local folder: {}".format(
                    os.path.join(project.local_path, folder.path)
                )
            )

            os.makedirs(os.path.join(project.local_path, folder.path), exist_ok=True)
        ...
        for file in new_files:
            ...
            download_folder = os.path.join(
                project.local_path, os.path.dirname(file.path)
            )
    ...
    def upload_new_assets(self, project):
        ...
        for folder in new_folders:
            if not os.path.isdir(os.path.join(project.local_path, folder.path)):
                logger.info("Can't find {}, skipping.".format(folder.name))
                folder.delete_instance()
                continue

            self.create_frameio_folder_tree(
                project=project, new_folder_path=folder.path
            )
        ...
        for file in new_files:
            ...
            abs_path = os.path.abspath(os.path.join(project.local_path, file.path))
    ...
    def delete_and_reupload(self, project, asset):
        ...
        if asset.upload_retries == 2:
            logger.info("Asset already uploaded 3 times. Marking as successful anyway")
        ...
        abs_path = os.path.abspath(os.path.join(project.local_path, asset.path))
        ...
        if os.path.dirname(asset.path) == "":
            ...
            parent_asset_id = self.Asset.get(
                self.Asset.project_id == project.project_id,
                self.Asset.path == os.path.dirname(asset.path),
            ).asset_id
    ...
    def verify_new_uploads(self):
        ...
        """
        new_assets = self.Asset.select().where((self.Asset.upload_verified == False))
    ...
    def update_ignored_assets(self):
        ...
        all_blocked_folders = self.Asset.select().where(
            (self.Asset.ignore == True) & (self.Asset.is_file == False)
        )
        ...
        for ignore_folder in removed_ignore_folders:
            ...
            blocked_folders = [
                f
                for f in all_blocked_folders
                if f.name == ignore_folder.name
                or self.wildcard_match(f.name, [ignore_folder.name])
            ]
        ...
        while True:
            if authenticated_client():
                try:
                    ...
                    logger.info(
                        "Could not connect, retrying in {}".format(config.SCAN_INTERVAL)
                    )
                ...
                if self.IgnoreFolder.select().where(self.IgnoreFolder.removed == True):
